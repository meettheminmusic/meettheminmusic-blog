---
title: Bim Bum Biddy
seo_title: "Bim Bum Biddy: Lyrics and Lesson Plan | Elementary Music"
description: "Free notation, lyrics, and an elementary lesson plan for the challenging hand-clapping game 'Bim Bum Biddy,' built to teach grit, in D Major, 2/4 meter."
date: 2026-04-19T21:23:00
draft: false
unlisted: false
featured: false
card_image: /images/uploads/bimbumbiddy.webp
tonal_concepts: []
rhythmic_concepts:
  - 1/16 notes
activity_types:
  - Hand game
social_concepts:
  - Grit
movement_concepts: []
supports_adaptations: []
language_origins: []
keys:
  - D
modes:
  - Major
meters:
  - Duple
  - 2/4
source: ''
source_url: ''
abc_notation: |-
  %%textfont arial italic 9
  X:1 
  T:Bim Bum Biddy
  M:2/4
  L:1/16
  C:arr. P. Eric Bottorff
  A: MeetThemInMusic.com 
  %%printtempo 0
  Q:1/4=100
  %%vocalfont Arial 9
  %%barsperstaff 4
  %%stretchlast 1
  K:D
  D2A2 D2A2| BAGB A2FG| A2GF GFE2| F2A2 z4||
  w: bim bum bim bum bid-dy bid-dy bum. Bi-dy bum bi-dy bi-dy bum bim bum.
  M: 2/4
  D2A2 D2A2| BAGB A2FG| A2GF GFE2| F2D2 "^Fine"z4||
  w: bim bum bim bum bid-dy bid-dy bum. Bi-dy bum bi-dy bi-dy bum bim bum.
  M: 2/4
  d4 A4 | BAGB A2FG| A2GF GFE2| F2A2 z4||
  w: bim bum  bid-dy bid-dy bum. Bi-dy bum bi-dy bi-dy bum bim bum.
  M:2/4
  d4 A4 | BAGB A2FG| A2GF GFE2| "^ D.C. al fine"F2D2 z4|]
  w: bim bum  bid-dy bid-dy bum. Bi-dy bum bi-dy bi-dy bum bim bum.
  %%textfont arial 9
  %%center .
  %%center © 2026 All Rights Reserved.
sanitized_abc: ''
abc_image: ''
abc_tempo: 120
abc_scores: []
---

<u>**Activity**</u>

I introduce this by saying "the points of this song is that it is challenging. It is hard to keep up with the hand movements!"  _Productive Struggle is important!<p>_

| Word | Movement |
| --- | --- |
| Bim | Both hand tap legs |
| Bum | Clap |
| Biddy Biddy | Snap or tap thumb and fingers |

> A lot of kids can't snap.   I call tapping the thumb and fingers "crab clapping."  This also helps with speed.

<u>**Extension**</u>
As the teacher, you get a ton of practice. Kids will notice and ask how you are able to do it so fast.  Turn it into a math question!
"I have been teaching this to 5 classes this week and I practiced it with you 10 times ... I got 50 practices this week. BUT! I have been teaching for 15 years ... "

<u>**Example Teaching**</u>

This video just shows the first few minutes of doing rote practice.  The goal is to talk to kids about what their brains are doing when they try hard things, start meta-cognition, and getting students to discover ways to push through the challenges.  I am not the "sage on the stage."

<iframe width="560" height="315" src="https://www.youtube.com/embed/RbfyJNrGorY?si=NMi4HTY2rOs3pxZQ" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

![movement cards for pat clap and snap](/images/uploads/Movement%20cards.png "movement cards for pat clap and snap")
