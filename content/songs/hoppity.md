---
title: Hoppity
date: 2026-09-20T14:20:00-04:00
draft: false
unlisted: false
featured: false
card_image: ''
tonal_concepts: []
rhythmic_concepts: []
activity_types: []
social_concepts: []
movement_concepts: []
supports_adaptations: []
language_origins: []
keys: []
modes: []
meters: []
source: ''
source_url: ''
panels: []
abc_notation: |-
  X:1
  T:Hoppity
  C:words by A.A. Milne
  C:arr. P. Eric Bottorff
  M:6/8
  L:1/8
  Q:3/8=120
  %%score 1 2 3
  V:1 clef=perc stafflines=1 stem=up
  V:2 clef=perc stafflines=1 nm="Shaker" snm="Sh."
  V:3 clef=perc stafflines=1 nm="Drum" snm="Dr."
  K:C
  [V:1] BBB BBB | BBB BBB |
  w: Chris-to-pher Ro-bin goes | Hopp-it-y, hopp-it-y, |
  [V:2] B3 B3 | B2 B B3 |
  w: Hop Hop | Hop don't stop. |
  [V:3] B2 B B3 | B3 BBB |
  w: Come a long. | Hop Chris-to-pher. |
  %
  [V:1] BBB BBB | B3 z2 B |
  w: Hopp-it-y, hopp-it-y, | hop. When-
  [V:2] B3 B3 | B2 B B3 |
  w: Hop Hop | Hop don't stop. |
  [V:3] B2 B B3 | BBB B3 |
  w: Come a long, | Chris-to-pher Stop! |
  %
  [V:1] BBB BBB | BBB BBB |
  w: ev-er I tell him pol- | ite-ly to stop it, he |
  [V:2] B3 B3 | B2 B B3 |
  w: Hop, Hop, | Hop! Don't stop! |
  [V:3] B2 B B3 | B3 BBB |
  w: Come a long. | Hop Chris-to-pher. |
  %
  [V:1] BBB BBB | B3 z3 |]
  w: Says he can't poss-ib-ly | stop. |
  [V:2] B3 B3 | B2 B B3 |]
  w: Hop, Hop, | Hop! Don't stop! |
  [V:3] B2 B B3 | BBB B3 |]
  w: Come a long, | Chris-to-pher Stop! |
  %%textfont arial 9
  %%center .
  %%center © 2026 All Rights Reserved.
sanitized_abc: ''
abc_image: ''
abc_tempo: 120
abc_scores:
  - label: Vocal Only
    notation: |-
      X:1
      T:Hoppity
      C:words by A.A. Milne
      C:arr. P. Eric Bottorff
      M:6/8
      L:1/8
      Q:3/8=120
      %%score 1 2 3
      V:1 clef=perc stafflines=1 stem=up
      K:C
      [V:1] BBB BBB | BBB BBB |
      w: Chris-to-pher Ro-bin goes | Hopp-it-y, hopp-it-y, |
      [V:1] BBB BBB | B3 z2 B |
      w: Hopp-it-y, hopp-it-y, | hop. When-
      [V:1] BBB BBB | BBB BBB |
      w: ev-er I tell him pol- | ite-ly to stop it, he |
      [V:1] BBB BBB | B3 z3 |]
      w: Says he can't poss-ib-ly | stop. |
      %%textfont arial 9
      %%center .
      %%center © 2026 All Rights Reserved.
    image: ''
    tempo: null
  - label: Shaker Only
    notation: |-
      X:1
      T:Hoppity
      C:words by A.A. Milne
      C:arr. P. Eric Bottorff
      M:6/8
      L:1/8
      Q:3/8=120
      %%score 1 2 3
      V:1 clef=perc stafflines=1 stem=up nm="Shaker"
      K:C
      [V:1] B3 B3 | B2 B B3 |
      w: Hop Hop | Hop don't stop. |
      [V:1] B3 B3 | B2 B B3 |
      w: Hop Hop | Hop don't stop. |
      [V:1] B3 B3 | B2 B B3 |
      w: Hop, Hop, | Hop! Don't stop! |
      [V:1] B3 B3 | B2 B B3 |]
      w: Hop, Hop, | Hop! Don't stop! |
      %%textfont arial 9
      %%center .
      %%center © 2026 All Rights Reserved.
    image: ''
    tempo: null
  - label: Drum Only
    notation: |-
      X:1
      T:Hoppity
      C:words by A.A. Milne
      C:arr. P. Eric Bottorff
      M:6/8
      L:1/8
      Q:3/8=120
      %%score 1 2 3
      V:1 clef=perc stafflines=1 stem=up nm="Drum"
      K:C
      [V:1] B2 B B3 | B3 BBB |
      w: Come a long. | Hop Chris-to-pher. |
      [V:1] B2 B B3 | BBB B3 |
      w: Come a long, | Chris-to-pher Stop! |
      [V:1] B2 B B3 | B3 BBB |
      w: Come a long. | Hop Chris-to-pher. |
      [V:1] B2 B B3 | BBB B3 |]
      w: Come a long, | Chris-to-pher Stop! |
      %%textfont arial 9
      %%center .
      %%center © 2026 All Rights Reserved.
    image: ''
    tempo: null
---
