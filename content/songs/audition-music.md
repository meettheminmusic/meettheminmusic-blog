---
title: Audition Music
seo_title: "Audition Music: Baritone and Trumpet | K-2 Music"
description: "Notated baritone and trumpet audition excerpts for instrument transposition practice, an internal teaching resource for general music programs."
date: 2026-04-19T16:05:00
draft: false
unlisted: true
featured: false
card_image: ''
tonal_concepts: []
rhythmic_concepts: []
activity_types: []
social_concepts: []
movement_concepts: []
supports_adaptations: []
language_origins: []
keys: []
modes: []
meters: []
source: ''
source_url: ''
abc_notation: ''
sanitized_abc: ''
abc_image: ''
abc_tempo: 60
abc_scores:
  - label: Baritone
    notation: |-
      %%textfont arial italic 11
      X:1 
      T:Baritone Audition
      M:3/4
      L:1/16
      C:
      A: MeetThemInMusic.com 
      %%gchordfont Arial 12
      %%vocalfont Arial 10
      %%staffsep 50
      %%barsperstaff 4
      %%stretchlast 1
      K:Bb clef=bass
      B,F,D,F, B,2B,c, D2B,2| C2A,B, C2A,2 F,4 | CA,F,A, C2CD E2C2| D2B,C D2B,2 C4| G,D,B,,D, G,2B,A, B,2G,2| F,2B,2 d,,E,F,2 B,,2D,2| C,3F, E6 A,2| B,A,B,2 CB,C2 B,4:||
      %%textfont arial 9
    image: ''
    tempo: 60
  - label: Trumpet
    notation: |-
      %%textfont arial italic 11
      X:1 
      T:Trumpet Audition
      M:3/4
      L:1/8
      A: MeetThemInMusic.com 
      %%gchordfont Arial 12
      %%vocalfont Arial 10
      %%staffsep 50
      %%barsperstaff 5
      %%stretchlast 1
      %%MIDI transpose -2
      K:Cm
      cG2c2e|dG2d2f|e c2e2g|fedc=BA|GC/2D/2 E/2D/2C cA| FD/2E/2 F/2E/D d=B | AF/2G/2 A/2G/2F fd| =Bd/2c/2 =B/2A/2G/2F/2|E C2
      %%textfont arial 9
    image: ''
    tempo: 70
---

It is possible the trumpet part will not sound quite right.  If the notes song wrong on the turmpet, set the transpose label to Bb and the octave label to -1
